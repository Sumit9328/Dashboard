...
export default function ClassStud() {
  const [selectedCountry, setSelectedCountry] = useState("Select Subject");
  ...
  const handleSelect = (country: string) => { // Explicitly typed 'country'
    setSelectedCountry(country);
    setIsOpen(false);
  };

  return (
    <div className="w-[120px] mx-auto mt-2">
      <label className="block text-[#B6B6B5] text-[14px] font-[100] mb-1">
      Subject
      </label>
      <div
        className="relative border border-[#333D61] rounded-[5px] shadow-sm bg-white"
        onClick={toggleDropdown}
      >
        <div className="p-1 flex justify-between items-center cursor-pointer">
          {/* Selected country with text size of 50px */}
          <span
            className="text-gray-700 font-normal"
            style={{ fontSize: "12px", color:'#333D61' }}
          >
            {selectedCountry}
          </span>
          <span
