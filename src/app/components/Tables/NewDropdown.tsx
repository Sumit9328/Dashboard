...
export default function NewDropdown() {
  const [selectedCountry, setSelectedCountry] = useState("Select Class");
...
<div className="w-44 mx-auto mt-2">
  <div
    className="relative border border-[#333D61] rounded-[6px] shadow-sm bg-white"
    onClick={toggleDropdown}
  >
    <div className="px-0 py-1 flex justify-around items-center cursor-pointer">
      {/* Update font size */}
      <span className="text-gray-700 font-normal text-[12px]">{selectedCountry}</span>
      <span
    ...
    {isOpen && (
      <ul className="absolute left-0 right-0 max-h-40 overflow-y-auto bg-white border border-[#333D61] rounded-lg shadow-lg mt-1 z-10">
        {countries.map((country, index) => (
          <li
            key={index}
            className="p-2 text-[10px] text-gray-700 hover:bg-[#F0F4FF] hover:text-[#333D61] cursor-pointer transition-colors"
